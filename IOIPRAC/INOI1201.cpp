...
int main(){
    ...
    VI cobol(N,0); //cobol time till now
    //applying insertion sort
    cobol[0]=v[0].first;
    LOOPL(i,1,N){
        int j=i;
        while(j>0){
            int before_swap=max(v[j-1].second,v[j].first+v[j].second);
            swap(v[j],v[j-1]);
            int after_swap=max(v[j-1].second,v[j].first+v[j].second);
            if(before_swap>after_swap){
                if(j-1>0) cobol[j-1]=cobol[j-2]+v[j-1].first;
                else cobol[j-1]=v[j-1].first;
                cobol[j]=cobol[j-1]+v[j].first;
            }
            else{
                swap(v[j],v[j-1]);
                break;
